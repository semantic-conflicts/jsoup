...
public class HttpConnection implements Connection {
  private static int HTTP_TEMP_REDIR=307;
...
@SuppressWarnings({"unchecked"}) private static abstract class Base<T extends Connection.Base> implements Connection.Base<T> {
    URL url;
    Method method;
...
public static class Response extends Base<Connection.Response> implements Connection.Response {
    ...
    private Connection.Request req;
    /** 
 * For example  {@code application/atom+xml;charset=utf-8}. Stepping through it: start with  {@code "application/"}, follow with word characters up to a  {@code "+xml"}, and then maybe more ( {@code .*}).
 */
    public static Pattern xmlContentTypeRxp=Pattern.compile("application/\\w+\\+xml.*");
    public Response(){
      super();
    }
    ...
    public static Response execute(    Connection.Request req,    Response previousResponse) throws IOException {
      ...
      try {
        ...
        if (status != HttpURLConnection.HTTP_OK) {
          if (status == HttpURLConnection.HTTP_MOVED_TEMP || status == HttpURLConnection.HTTP_MOVED_PERM || status == HttpURLConnection.HTTP_SEE_OTHER || status == HTTP_TEMP_REDIR)           needsRedirect=true;
 else           if (!req.ignoreHttpErrors())           throw new HttpStatusException("HTTP error fetching URL",status,req.url().toString());
        }
        ...
        if (contentType != null && !req.ignoreContentType() && !contentType.startsWith("text/") && !contentType.startsWith("application/xml") && !xmlContentTypeRxp.matcher(contentType).matches())         throw new UnsupportedMimeTypeException("Unhandled content type. Must be text/*, application/xml, or application/xhtml+xml",contentType,req.url().toString());
      ...
      for (      Map.Entry<String,List<String>> entry : resHeaders.entrySet()) {
        ...
        if (name.equalsIgnoreCase("Set-Cookie")) {
          for (          String value : values) {
            ...
            if (cookieName != null && cookieName.length() > 0)             cookie(cookieName,cookieVal);
          }
        }
 else {
    ...
    public static void writePost(    Collection<Connection.KeyVal> data,    OutputStream outputStream) throws IOException {
