...
public class HttpConnection implements Connection {
  ...
  /** 
 * Many users would get caught by not setting a user-agent and therefore getting different responses on their desktop vs in jsoup, which would otherwise default to   {@code Java}. So by default, use a desktop UA.
 */
  public static String DEFAULT_UA="Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/53.0.2785.143 Safari/537.36";
...
@SuppressWarnings({"unchecked"}) public static abstract class Base<T extends Connection.Base> implements Connection.Base<T> {
    ...
    public static String fixHeaderEncoding(    String val){
      try {
 ...
 */
    ...
    public String getHeaderCaseInsensitive(    String name){
      ...
      if (value == null)       value=headers.get(lowerCase(name));
      if (value == null) {
        Map.Entry<String,String> entry=scanHeaders(name);
